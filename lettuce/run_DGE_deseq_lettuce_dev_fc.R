# Name: run_DGE_deseq_lettuce_dev_fc.R
# Author: EY
# Date: 08/29/2023
# Version:4.2.1
# Description: Will run the differential gene expression on developmental stages in lettuce
...
counts<-read.csv('lettuce/raw_lettuce_counts.csv', row.names=1)
...
counts_deseq<-DESeqDataSetFromMatrix((counts),colData = metadata, design=~0+dev_stage)

# run the DGE analysis
DESeq_dataset_results<-DESeq(counts_deseq,parallel=TRUE)
