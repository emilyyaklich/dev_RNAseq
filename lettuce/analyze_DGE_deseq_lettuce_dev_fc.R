# Name: analyze DGE deseq lettuce fc
# Author: EY (based off of code written by E. Dittmar)
# Date: 08/29/2023 
# Version:4.2.1
# Description: Will analyze the output from the DESeq DGE for pairwise dev stages
...
ma_plot<-ggplot(ma_data, aes(x = mean, y = lfc, color = significant)) +
  geom_point(alpha = 0.4) +
  scale_color_manual(values = c("up" = "hotpink", "down" = "brown", "not_sig" = "grey")) +
  labs(title = "VMvTM",
       x = "Mean of Normalized Counts",
       y = "Log2 Fold Change") +
  theme_minimal() +
...
ggsave("lettuce/plots/maplot_VMvTM_pinkBrown.png", plot = ma_plot, dpi = 300, width=5.02, height=2.84)
# width=3, height=6 for daniel figures
...
ggsave("lettuce/plots/maplot_TMvIM_pinkBrown.png", plot = ma_plot, dpi = 300,width=5.02, height=2.84)
...
ggsave("lettuce/plots/maplot_IMvIMFM_pinkBrown.png", plot = ma_plot, dpi = 300,width=5.02, height=2.84)
