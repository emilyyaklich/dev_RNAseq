...
geom_line(size=1)+ geom_point(size=2)+ ggtitle(plot_title) + annotate('text',x="35D",y=0.2,label=dev_sig, size =2)+ ylim(-0.5,0.5)+theme_bw() + theme(plot.title = element_text(size=8),axis.line=element_line(colour = "black"),
...
ggplot(data=plotting_df, aes(x=dev_stage, y=eigenval, group=1)) + geom_ribbon(aes(ymin = module_subset$SE.X10D, ymax = module_subset$SE.X10D), fill = "grey70") +
  geom_line(size=1)+ geom_point(size=2)+ ggtitle(plot_title) + annotate('text',x="35D",y=0.2,label=dev_sig, size =2)+ ylim(-0.5,0.5)+theme_bw() + theme(plot.title = element_text(size=8),axis.line=element_line(colour = "black"),
                                                                                                                                                        panel.grid.major=element_blank(),
                                                                                                                                                        panel.grid.minor = element_blank(),
                                                                                                                                                        panel.border=element_blank(),
...
geom_ribbon(aes(ymin = module_subset$SE.X10D/2, ymax = module_subset$SE.X10D/2), fill = "grey70")
...
for (module in module_names) {
  j <- j+1
  path_to_plots<-paste('sunflower/wgcna/plots/change_plots/individual_plots/',module,".png", sep="")
...
png('sunflower/wgcna/plots/change_plots/subset1.png', width=2000, height =2200, res=300)
plot_1 <- do.call(grid.arrange, subset_1)
dev.off()


subset_2 <- plot_list[13:24]
png('sunflower/wgcna/plots/change_plots/subset2.png', width=2000, height =2200, res=300)
plot_2 <- do.call(grid.arrange, subset_2)
dev.off()

subset_3<- plot_list[25:36]
png('sunflower/wgcna/plots/change_plots/subset3.png', width=2000, height =2200, res=300)
plot_3<-do.call(grid.arrange, subset_3)
dev.off()

subset_4<- plot_list[37:48]
png('sunflower/wgcna/plots/change_plots/subset4.png', width=2000, height =2200, res=300)
plot_4<-do.call(grid.arrange, subset_4)
dev.off()

subset_5 <- plot_list[49:55]
png('sunflower/wgcna/plots/change_plots/subset5.png', width=2000, height =2200, res=300)
...
subset_expression <- input_mat_filt_t[genes_in_module, ]
...
# Calculate row-wise averages in groups of three columns
row_avg_values <- rowMeans(subset_expression[, seq(2, ncol(subset_expression), by = 3)], na.rm = TRUE)

# Create a new dataframe with averaged values
avg_df <- cbind(subset_expression[, 1, drop = FALSE], Average_Expression = row_avg_values)

# Plotting with ggplot2
ggplot(melted_data, aes(x = Var2, y = Expression, group = Var1)) +
  geom_line() +
  geom_point() +
  geom_smooth(method="loess", se=FALSE, aes(group=1))
  scale_x_discrete(labels = colnames(subset_expression)[-1]) +  # Use column names as x-axis labels, excluding GeneID
  scale_y_continuous(limits=c(0,50))+
  labs(title = "Expression Changes for Genes in",
       x = "dev_stage",
       y = "Expression") +
