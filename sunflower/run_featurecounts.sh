...
featureCounts -T 8 -p -B -C -s 2 --tmpDir /scratch/ely67071/feature_tmp  -a "$ANNOTATION_DIR/Ha412HO_v2_braker.gtf" -o "$OUTPUT_DIR/sunflower_dev_fc_gtf_test.txt" "$BAM_FILE_DIR"/*.bam
...
# -s 2 → stranded library (reverse stranded reads), because the library was prepared using the stranded mRNA-seq kit (KAPA Biosystems)
